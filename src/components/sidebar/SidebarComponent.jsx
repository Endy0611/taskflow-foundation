              ...
              >
                ...
                <div className="max-h-60 overflow-y-auto py-1">
                  {loading ? (
                    ...
                    workspaces.map((w) => {
                      const id = String(w.id ?? w.workspaceId);
                      const active = id === String(currentWsId);
                      return (
                        <button
                          key={id}
                          className={`w-full flex items-center gap-2 px-2 py-2 text-left text-sm
                            ${active ? "bg-blue-50 dark:bg-gray-800 font-medium" : "hover:bg-gray-100 dark:hover:bg-gray-800"}`}
                        ...
                        >
                          ...
                          <span className="truncate flex-1">
                            {w.name}
                            {w.theme ? <span className="opacity-70"> · {w.theme}</span> : null}
                          </span>
                          {active ? <Dot className="w-5 h-5 text-blue-600" /> : null}
                        </button>
                      );
                    })
                  )}
